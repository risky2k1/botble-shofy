<section class="tp-product-area position-relative pt-30 pb-30">
    <div class="container">
        <div class="row align-items-center mb-40">
            <div @class([
                'col-xl-5 col-lg-6 col-md-5' => count($selectedTabs) > 1,
                'col-12' => count($selectedTabs) <= 1,
            ])>
                {!! Theme::partial('section-title', compact('shortcode')) !!}
            </div>
            <div @class([
                'col-xl-7 col-lg-6 col-md-7' => count($selectedTabs) > 1,
                'd-none' => count($selectedTabs) <= 1,
            ])>
                    ...
                    <ul class="nav nav-tabs justify-content-sm-end" id="productTab" role="tablist"
                        ...
                        @foreach ($productTabs as $key => $tab)
                            ...
                            <li class="nav-item" role="presentation">
                                <button @class(['nav-link', 'active' => $loop->first]) id="{{ $key }}-tab" data-bs-toggle="tab"
                                    data-bs-target="#tab-pane" type="button" role="tab" aria-controls="tab-pane"
                                    @if ($loop->first) aria-selected="true" @endif
                                    data-bb-toggle="product-tab" data-bb-value="{{ $key }}">
                                    {{ $tab }}
